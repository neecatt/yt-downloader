...
export function Dashboard({ initialData }: { initialData: ActivityResponse | null }) {
  ...
  return <AdminLayout><main className="dashboard-shell">
    ...
    <section className="panel">
      <div className="filters"><input aria-label="Search activity" placeholder="Search username, title, or link" value={q} onChange={(event) => setQ(event.target.value)} onKeyDown={(event) => event.key === "Enter" && refresh()} /><select aria-label="Filter platform" value={platform} onChange={(event) => setPlatform(event.target.value)}><option value="">All platforms</option><option value="youtube">YouTube</option><option value="instagram">Instagram</option><option value="facebook">Facebook</option><option value="tiktok">TikTok</option><option value="x">X</option><option value="linkedin">LinkedIn</option></select><select aria-label="Filter status" value={status} onChange={(event) => setStatus(event.target.value)}><option value="">All statuses</option><option value="completed">Completed</option><option value="failed">Failed</option><option value="started">Started</option><option value="cancelled">Cancelled</option></select><button className="button" onClick={refresh} disabled={loading}>Apply</button></div>
