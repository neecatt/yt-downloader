...
export async function updateUser(userId:number,payload:{creditAdjustment?:number;creditMode?:"add"|"set";creditAmount?:number;aiTrials?:number;complimentary?:boolean;reason?:string}) { const response=await fetch(apiUrl(`/admin/users/${userId}`),{method:"PATCH",headers:headers(true),body:JSON.stringify(payload),cache:"no-store",signal:AbortSignal.timeout(8000)}); if(!response.ok) throw new Error((await response.json().catch(()=>null))?.detail||"Could not update user"); return await response.json(); }
