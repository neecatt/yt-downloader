...
export const fetchUsage = (days: number, excludedUsers: string[] = []) => get<UsageData>(`/admin/usage?days=${Math.min(365, Math.max(1, days))}${excludedUsers.length ? `&excludeUsers=${encodeURIComponent(excludedUsers.join(","))}` : ""}`);
