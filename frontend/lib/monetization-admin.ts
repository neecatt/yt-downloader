...
export type TransactionData = { transactions: { id: string; userId: number; delta: number; balanceAfter: number; reason: string; operationId: string | null; actor: string; createdAt: string }[]; page: number; pageSize: number; total: number };
...
export const fetchReferrals = (status: string, page: number, pageSize = 25) => get<ReferralData>(`/admin/referrals?status=${encodeURIComponent(status)}&page=${page}&pageSize=${pageSize}`);
